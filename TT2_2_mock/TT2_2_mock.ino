...
class AccelStepper { 
 public:
  AccelStepper(int i, int b, int c) {}
  void setMaxSpeed(long i){ m=i; }
  void run() { 
    if(t==c) return;
    if((t-c)>0) c++;
    if((t-c)<0) c--;
  ...
  void setAcceleration(long i){}
  void moveTo(long i) { t=i; }
  void setCurrentPosition(long a) { c=a;}
  long distanceToGo() { return t-c; }
 private:
  long m, s, c, t;
};
...
class Keypad {
 public:
  Keypad(char* x, byte* a, byte* b, byte c, byte d) {}
  char getKey(){ 
    if(Serial.available()) return Serial.read();
    else return NO_KEY;
  }
};
char* u8g_font_profont11;
class u8g {
 public: 
  u8g(){}
  u8g(int a, int b, int c, int d, int e){}
  firstPage(){}
  drawHLine(int x, int y, int w){}
  drawVLine(int x, int y, int h){}
  setFont(void* u8g_font_profont11){}
  drawStr(int x, int y, String s){ Serial.print("\n"); Serial.print(s); }
  setPrintPos(int x, int y){}
  print(String x) { Serial.print(x); }
  bool nextPage(){ return false; }
};
class U8GLIB_PCD8544 : public u8g {
 public:
...
//========================================================================
 ...
 byte enable = LOW;       // the stepper is enabled when StepperEnable is low or 0
 ...
 #define StepperEnable 2  // Arduino pin 2 to EasyDriver Enable
 #define MS1 41           // Arduino pin 41 to EasyDriver MS1
 #define MS2 43           // Arduino pin 43 to EasyDriver MS2
 ...
 void setup(void) {
      #ifdef Mock
          Serial.begin(115200);
          Serial.print("'nTT2_2_Mock");
          microStepsPerFullStep = 1;
      #endif
     ...
     */
      ...
      digitalWrite(StepperEnable,enable);
...
void loop(){
 ...
 // At a destination?
  ...
  drawnokiascreen( sTargetTrack );
      stepper.moveTo( microPosn );                   // go to new position
      sCurrentPosition = String("mvng");           // indicate a move is in progress
