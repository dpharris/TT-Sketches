#include <AccelStepper.h>   // AccelStepper Library
 ...
 #define NUMTRACKS 16
 long steps[NUMTRACKS] = {0,240,480,720,1200,1680,1920,2160,2400,2760,3000,3840,4080,4800,6120,7800};
 const int microStepsPerFullStep = 1;
 ...
 */
  ...
  digitalWrite(MS1, HIGH);  // LOW LOW = Full Step, HIGH HIGH = 1/8 Step
...
void loop(){
  ...
  if (keypressed != NO_KEY){  // If keypad button pressed check which key
    if( keypressed == 'A' ) {
      ...
      stepper.moveTo( microPosn );                          // go to ne position
      ...
      if( ikey>=0 && ikey<=9 ) {                            // is 0-9, then
        ...
        if(enteredTrack>NUMTRACKS) enteredTrack = NUMTRACKS;        // not past the last track
